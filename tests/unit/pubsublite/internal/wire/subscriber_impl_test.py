...
from google.cloud.pubsublite_v1 import SubscribeRequest, SubscribeResponse, InitialSubscribeRequest, FlowControlRequest, \
  SeekRequest
...
def default_connection():
  conn = MagicMock(spec=Connection[SubscribeRequest, SubscribeResponse])
...
def connection_factory(default_connection):
  factory = MagicMock(spec=ConnectionFactory[SubscribeRequest, SubscribeResponse])
...
async def test_basic_flow_control_after_timeout(
    subscriber: Subscriber, default_connection, initial_request, asyncio_sleep, sleep_queues):
  ...
  async with subscriber:
    ...
    await flow_fut1
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(flow_1))])
    ...
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(flow_1)), call(
      as_request(FlowControlRequest(allowed_messages=15, allowed_bytes=15)))])


async def test_flow_resent_on_restart(subscriber: Subscriber, default_connection, initial_request, asyncio_sleep,
                                      sleep_queues):
  write_called_queue = asyncio.Queue()
  write_result_queue = asyncio.Queue()
  flow_1 = FlowControlRequest(allowed_messages=100, allowed_bytes=100)
  ...
  async with subscriber:
    ...
    await write_called_queue.get()
    await write_result_queue.put(None)
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(flow_1)), call(initial_request),
                                               call(as_request(
                                                 FlowControlRequest(allowed_messages=115, allowed_bytes=115)))])


async def test_message_receipt(subscriber: Subscriber, default_connection, initial_request, asyncio_sleep,
                               sleep_queues):
  write_called_queue = asyncio.Queue()
  write_result_queue = asyncio.Queue()
  ...
  async with subscriber:
    ...
    await write_result_queue.put(None)
    await flow_fut
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(flow))])
    ...
    # Reinitialization
    await write_called_queue.get()
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(flow)), call(initial_request)])
    await write_result_queue.put(None)
    await read_called_queue.get()
    await read_result_queue.put(SubscribeResponse(initial={}))
    # Sends fetch offset seek on the stream, and checks the response.
    seek_req = SubscribeRequest(seek=SeekRequest(cursor=Cursor(offset=message_2.cursor.offset + 1)))
    await write_called_queue.get()
    default_connection.write.assert_has_calls(
      [call(initial_request), call(as_request(flow)), call(initial_request), call(seek_req)])
    await write_result_queue.put(None)
    await read_called_queue.get()
    ...
    default_connection.write.assert_has_calls(
      [call(initial_request), call(as_request(flow)), call(initial_request), call(seek_req),
       call(as_request(FlowControlRequest(allowed_messages=98, allowed_bytes=85)))])


async def test_out_of_order_receipt_failure(subscriber: Subscriber, default_connection, initial_request, asyncio_sleep,
                                            sleep_queues):
  write_called_queue = asyncio.Queue()
  write_result_queue = asyncio.Queue()
  flow = FlowControlRequest(allowed_messages=100, allowed_bytes=100)
