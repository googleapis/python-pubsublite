...
async def test_permanent_error_on_reinitializer(retrying_connection: Connection[int, int], reinitializer,
                                                default_connection):
  fut = asyncio.Future()
...
async def test_successful_reinitialize(retrying_connection: Connection[int, int], reinitializer,
                                       default_connection):
  fut = asyncio.Future()
  reinitialize_called = asyncio.Future()
  ...
  async with retrying_connection as _:
    ...
    assert default_connection.read.call_count == 2  # re-call to read once first completes
...
async def test_reinitialize_after_retryable(retrying_connection: Connection[int, int], reinitializer,
                                            default_connection, asyncio_sleep):
