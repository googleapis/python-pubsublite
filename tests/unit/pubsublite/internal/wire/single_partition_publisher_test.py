...
from google.cloud.pubsublite_v1.types.publisher import InitialPublishRequest, PublishRequest, PublishResponse, \
...
def publisher(connection_factory, initial_request):
  return SinglePartitionPublisher(initial_request.initial_request, FLUSH_SECONDS, connection_factory)
...
def as_publish_response(start_cursor: int):
  return PublishResponse(message_response=MessagePublishResponse(start_cursor=Cursor(offset=start_cursor)))


async def test_basic_publish_after_timeout(publisher: Publisher, default_connection, initial_request, asyncio_sleep,
                                           sleep_queues):
  ...
  async with publisher:
    ...
    await write_future
    default_connection.write.assert_has_calls([call(initial_request), call(as_publish_request([message1, message2]))])
    assert not publish_fut1.done()
    assert not publish_fut2.done()
...
async def test_publishes_multi_cycle(publisher: Publisher, default_connection, initial_request, asyncio_sleep,
                                     sleep_queues):
  sleep_called = sleep_queues[FLUSH_SECONDS].called
  sleep_results = sleep_queues[FLUSH_SECONDS].results
  message1 = PubSubMessage(data=b"abc")
  ...
  async with publisher:
    ...
    default_connection.write.assert_has_calls([call(initial_request), call(as_publish_request([message1]))])
    assert not publish_fut1.done()
...
async def test_publishes_retried_on_restart(publisher: Publisher, default_connection, initial_request, asyncio_sleep,
                                            sleep_queues):
  sleep_called = sleep_queues[FLUSH_SECONDS].called
  sleep_results = sleep_queues[FLUSH_SECONDS].results
  message1 = PubSubMessage(data=b"abc")
