...
from google.cloud.pubsublite_v1.types.cursor import StreamingCommitCursorRequest, StreamingCommitCursorResponse, \
...
def default_connection():
  conn = MagicMock(spec=Connection[StreamingCommitCursorRequest, StreamingCommitCursorResponse])
...
def connection_factory(default_connection):
  factory = MagicMock(spec=ConnectionFactory[StreamingCommitCursorRequest, StreamingCommitCursorResponse])
...
@pytest.fixture()
def initial_request():
  return StreamingCommitCursorRequest(initial=InitialCommitCursorRequest(subscription="mysub"))
...
async def test_basic_commit_after_timeout(
    committer: Committer, default_connection, initial_request, asyncio_sleep, sleep_queues):
  ...
  async with committer:
    ...
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(cursor2))])
    assert not commit_fut1.done()
    assert not commit_fut2.done()
...
async def test_commits_multi_cycle(committer: Committer, default_connection, initial_request, asyncio_sleep,
                                   sleep_queues):
  sleep_called = sleep_queues[FLUSH_SECONDS].called
  sleep_results = sleep_queues[FLUSH_SECONDS].results
  cursor1 = Cursor(offset=321)
  ...
  async with committer:
    ...
    await write_called_queue.get()
    await write_result_queue.put(None)
    default_connection.write.assert_has_calls([call(initial_request), call(as_request(cursor1))])
    assert not commit_fut1.done()
    ...
    default_connection.write.assert_has_calls(
      [call(initial_request), call(as_request(cursor1)), call(as_request(cursor2))])
    assert not commit_fut1.done()
    assert not commit_fut2.done()
...
async def test_publishes_retried_on_restart(committer: Committer, default_connection, initial_request, asyncio_sleep,
                                            sleep_queues):
  sleep_called = sleep_queues[FLUSH_SECONDS].called
  sleep_results = sleep_queues[FLUSH_SECONDS].results
  cursor1 = Cursor(offset=321)
  ...
  async with committer:
    ...
    asyncio_sleep.assert_has_calls(
      [call(FLUSH_SECONDS), call(FLUSH_SECONDS), call(FLUSH_SECONDS), call(_MIN_BACKOFF_SECS)])
    default_connection.write.assert_has_calls([
