...
def test_subscribe_transform_correct():
  expected = PubsubMessage(data=b'xyz', ordering_key='def', attributes={'x': 'abc', 'y': 'abc', PUBSUB_LITE_EVENT_TIME: encode_attribute_event_time(Timestamp(seconds=55).ToDatetime())}, message_id=str(10), publish_time=Timestamp(seconds=10))
  result = to_cps_subscribe_message(SequencedMessage(
      message=PubSubMessage(data=b'xyz', key=b'def', event_time=Timestamp(seconds=55), attributes={'x': AttributeValues(values=[b'abc']), 'y': AttributeValues(values=[b'abc'])}),
      publish_time=Timestamp(seconds=10), cursor=Cursor(offset=10), size_bytes=10))
...
def test_publish_valid_transform():
  expected = PubSubMessage(data=b'xyz', key=b'def', event_time=Timestamp(seconds=55), attributes={'x': AttributeValues(values=[b'abc']), 'y': AttributeValues(values=[b'abc'])})
  result = from_cps_publish_message(PubsubMessage(data=b'xyz', ordering_key='def', attributes={'x': 'abc', 'y': 'abc', PUBSUB_LITE_EVENT_TIME: encode_attribute_event_time(Timestamp(seconds=55).ToDatetime())}))
  assert result == expected
