...
def transformer():
  ...
  result.transform.side_effect = lambda source: PubsubMessage(message_id=str(source.cursor.offset))
  return result


@pytest.fixture()
def subscriber(underlying, flow_control_settings, ack_set_tracker, nack_handler, transformer):
  return SinglePartitionSubscriber(underlying, flow_control_settings, ack_set_tracker, nack_handler, transformer)
...
async def test_ack(subscriber: AsyncSubscriber, underlying, transformer, ack_set_tracker):
  ack_called_queue = asyncio.Queue()
  ack_result_queue = asyncio.Queue()
  ack_set_tracker.ack.side_effect = make_queue_waiter(ack_called_queue, ack_result_queue)
  async with subscriber:
...
async def test_track_failure(subscriber: SinglePartitionSubscriber, underlying, transformer, ack_set_tracker):
  async with subscriber:
...
async def test_ack_failure(subscriber: SinglePartitionSubscriber, underlying, transformer, ack_set_tracker):
  ack_called_queue = asyncio.Queue()
  ack_result_queue = asyncio.Queue()
  ack_set_tracker.ack.side_effect = make_queue_waiter(ack_called_queue, ack_result_queue)
  async with subscriber:
...
async def test_nack_failure(subscriber: SinglePartitionSubscriber, underlying, transformer, ack_set_tracker, nack_handler):
  async with subscriber:
    message = SequencedMessage(cursor=Cursor(offset=1), size_bytes=5)
    underlying.read.return_value = message
    read: Message = await subscriber.read()
...
async def test_nack_calls_ack(subscriber: SinglePartitionSubscriber, underlying, transformer, ack_set_tracker, nack_handler):
  ack_called_queue = asyncio.Queue()
  ack_result_queue = asyncio.Queue()
  ack_set_tracker.ack.side_effect = make_queue_waiter(ack_called_queue, ack_result_queue)
  async with subscriber:
