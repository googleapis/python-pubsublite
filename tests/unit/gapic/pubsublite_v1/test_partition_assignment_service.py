...
def test__read_environment_variables():
    assert PartitionAssignmentServiceClient._read_environment_variables() == (
        False,
        "auto",
        None,
    )
...
def test__get_client_cert_source():
    ...
    with mock.patch(
        ...
        with mock.patch(
            ...
            assert (
                PartitionAssignmentServiceClient._get_client_cert_source(None, True)
...
@mock.patch.object(
    ...
    modify_default_endpoint_template(PartitionAssignmentServiceClient),
)
@mock.patch.object(
    PartitionAssignmentServiceAsyncClient,
    "_DEFAULT_ENDPOINT_TEMPLATE",
    modify_default_endpoint_template(PartitionAssignmentServiceAsyncClient),
)
def test__get_api_endpoint():
    ...
    mock_endpoint = PartitionAssignmentServiceClient._DEFAULT_ENDPOINT_TEMPLATE.format(
        UNIVERSE_DOMAIN=mock_universe
    )

    assert (
        PartitionAssignmentServiceClient._get_api_endpoint(
    ...
    assert (
        ...
        == PartitionAssignmentServiceClient.DEFAULT_MTLS_ENDPOINT
    )
    assert (
        PartitionAssignmentServiceClient._get_api_endpoint(
...
def test__get_universe_domain():
    ...
    assert (
        PartitionAssignmentServiceClient._get_universe_domain(
            client_universe_domain, universe_domain_env
        )
        == client_universe_domain
    )
    assert (
        PartitionAssignmentServiceClient._get_universe_domain(None, universe_domain_env)
        == universe_domain_env
    )
    assert (
        PartitionAssignmentServiceClient._get_universe_domain(None, None)
        == PartitionAssignmentServiceClient._DEFAULT_UNIVERSE
    )
...
def test_partition_assignment_service_client_from_service_account_file(
    client_class, transport_name
):
    creds = ga_credentials.AnonymousCredentials()
    with mock.patch.object(
...
def test_partition_assignment_service_client_get_mtls_endpoint_and_cert_source(
    client_class,
):
    mock_client_cert_source = mock.Mock()
...
def test_partition_assignment_service_client_create_channel_credentials_file(
    client_class, transport_class, transport_name, grpc_helpers
):
    # Check the case credentials file is provided.
    options = client_options.ClientOptions(credentials_file="credentials.json")
...
def test_assign_partitions_use_cached_wrapped_rpc():
    ...
    with mock.patch("google.api_core.gapic_v1.method.wrap_method") as wrapper_fn:
        ...
        client._transport._wrapped_methods[
            client._transport.assign_partitions
        ] = mock_rpc
        request = [{}]
...
@pytest.mark.asyncio
async def test_assign_partitions_async_use_cached_wrapped_rpc(
    ...
    with mock.patch("google.api_core.gapic_v1.method_async.wrap_method") as wrapper_fn:
        ...
        # Ensure method has been cached
        assert (
            client._client._transport.assign_partitions
...
def test_transport_kind_grpc_asyncio():
    transport = PartitionAssignmentServiceAsyncClient.get_transport_class(
        "grpc_asyncio"
    )(credentials=async_anonymous_credentials())
...
def test_partition_assignment_service_transport_create_channel(
    transport_class, grpc_helpers
):
    # If credentials and host are not provided, the transport class should use
    # ADC credentials.
