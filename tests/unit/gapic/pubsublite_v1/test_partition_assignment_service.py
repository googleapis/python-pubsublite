...
def test_assign_partitions(transport: str = "grpc"):
    ...
    request = subscriber.PartitionAssignmentRequest()

    requests = [request]

    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client._transport.assign_partitions), "__call__"
    ) as call:
...
@pytest.mark.asyncio
async def test_assign_partitions_async(transport: str = "grpc_asyncio"):
    ...
    # Mock the actual call within the gRPC stub, and fake the request.
    with mock.patch.object(
        type(client._client._transport.assign_partitions), "__call__"
    ) as call:
        # Designate an appropriate return value for the call.
...
def test_transport_grpc_default():
    ...
    assert isinstance(
        client._transport, transports.PartitionAssignmentServiceGrpcTransport,
    )
...
def test_partition_assignment_service_grpc_transport_channel_mtls_with_client_cert_source(
...
def test_partition_assignment_service_grpc_asyncio_transport_channel_mtls_with_client_cert_source(
    grpc_create_channel, grpc_ssl_channel_cred
):
    # Check that if channel is None, but api_mtls_endpoint and client_cert_source
    # are provided, then a mTLS channel will be created.
...
@mock.patch("google.api_core.grpc_helpers.create_channel", autospec=True)
def test_partition_assignment_service_grpc_transport_channel_mtls_with_adc(
...
@mock.patch("google.api_core.grpc_helpers_async.create_channel", autospec=True)
def test_partition_assignment_service_grpc_asyncio_transport_channel_mtls_with_adc(
    grpc_create_channel, api_mtls_endpoint
):
    # Check that if channel and client_cert_source are None, but api_mtls_endpoint
    # is provided, then a mTLS channel will be created with SSL ADC.
