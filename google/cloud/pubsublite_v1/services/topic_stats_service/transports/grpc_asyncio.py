...
class TopicStatsServiceGrpcAsyncIOTransport(TopicStatsServiceTransport):
    ...
    def compute_message_stats(
        ...
        if "compute_message_stats" not in self._stubs:
            self._stubs["compute_message_stats"] = self._logged_channel.unary_unary(
    ...
    def compute_head_cursor(
        ...
        if "compute_head_cursor" not in self._stubs:
            self._stubs["compute_head_cursor"] = self._logged_channel.unary_unary(
    ...
    def compute_time_cursor(
        ...
        if "compute_time_cursor" not in self._stubs:
            self._stubs["compute_time_cursor"] = self._logged_channel.unary_unary(
    ...
    def _prep_wrapped_messages(self, client_info):
        ...
        self._wrapped_methods = {
            self.compute_message_stats: self._wrap_method(
                self.compute_message_stats,
                default_timeout=None,
                client_info=client_info,
            ),
            self.compute_head_cursor: self._wrap_method(
                self.compute_head_cursor,
                default_timeout=None,
                client_info=client_info,
            ),
            self.compute_time_cursor: self._wrap_method(
